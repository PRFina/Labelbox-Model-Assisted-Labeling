...
API_KEY = None
ANNOTATION_PROJECT = "mal-video-classifications"
MAL_START_FRAME = 1
MAL_END_FRAME = 20
MAL_SKIP_FRAME = 1
...
if __name__ == "__main__":
   ...
   # For classification
    ontology_builder = lb.OntologyBuilder(
        classifications=[
            lb.Classification(
                ...
                options=[
                    lb.Option(value="bunny"),
                    lb.Option(value="tree"),
                    lb.Option(value="butterfly"),
                ]
            )
        ]
    )
    ...
    choices = list(set((instance.class_name for instance in class_instances.values())))
    for frame_idx in list(range(MAL_START_FRAME, MAL_END_FRAME, MAL_SKIP_FRAME)):
        
        answers = np.random.choice(choices, size=2, replace=False)
        ...
        video_classifications.append(
            lb_types.VideoClassificationAnnotation(
                ...
                value=lb_types.Checklist(
                    answer=[
                        lb_types.ClassificationAnswer(name = answer) for answer in answers
                    ]
                )
            )
        )
    ...
    label = lb_types.Label(
        ...
        annotations = video_classifications
    )
